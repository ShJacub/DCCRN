...
def cal_pesq(dirty_wavs, clean_wavs):
    # print(dirty_wavs.shape, clean_wavs.shape)
    pesq_scores = []
    for i in range(len(dirty_wavs)):
        # pesq_score = pesq(cfg.FS, clean_wavs[i], dirty_wavs[i], "wb")
        pesq_score = pesq(cfg.fs, clean_wavs[i][0], dirty_wavs[i], "wb")
...
def cal_stoi(dirty_wavs, clean_wavs):
    ...
    for i in range(len(dirty_wavs)):
        ...
        stoi_score = stoi(clean_wavs[i][0], dirty_wavs[i], cfg.fs, extended=False)
